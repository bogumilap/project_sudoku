...
def checkDone(uid, sudoku_id, sudoku):
    ...
    if end and checkCorrectCalc(sudoku_id, user_sudoku):
        firebase_sudoku.finishGame(uid, sudoku_id)


def checkCorrectDB(sudoku_id, sudoku):
...
def checkCorrectCalc(sudoku_id, sudoku):
    unsolved_sudoku = firebase_ref.getRef().child("unsolved_sudoku").child(sudoku_id).child("numbers").get()
    if not checkRows(unsolved_sudoku, sudoku): return False
    if not checkCols(unsolved_sudoku, sudoku): return False
    return checkSquares(unsolved_sudoku, sudoku)
...
def checkCols(unsolved_sudoku, users_sudoku):
    for i in range(9):
        ...
        for j in range(1, 10):
            if col.count(i) + col.count(str(i)) > 1:
                return False
    return True
            ...
            for row in range(3*i, 3*i + 3):
                for col in range(3*j, 3*j + 3):
                    if unsolved_sudoku[row][col] != 0:
                        ...
                        ind += 1
            for k in range(1, 10):
                if square.count(k) + square.count(k) > 1:
                    return False
    return True
